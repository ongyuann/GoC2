...
func ParseCoff(coff []byte) (string, error) {
	...
	var result string
	if winapi.GetConsoleWindow() == 0 {
		result, err = winapi.ExecuteFunctionSaveOutputGUI(unsafe.Pointer(entryPoint))
		if err != nil {
			cleanup(allocationAddresses)
			return "", err
		}
	} else {
		result, err = winapi.ExecuteFunctionSaveOutputConsole(unsafe.Pointer(entryPoint))
		if err != nil {
			cleanup(allocationAddresses)
			return "", err
		}
