...
func ConvertAssemblyFromFile(donutArgs string, c *ishell.Context) error {
	...
	if len(d) == 0 {
		c.Println(parser.Usage(errors.New("No Args Provided")))
		return errors.New("No Args Provided")
	}
	if err1 != nil {
		return err1
	}
	if err := parser.Parse(d); err != nil || *srcFile == "" {
		c.Println(parser.Usage(err))
		return err
	}
	if *srcFile == "" {
		return errors.New("No source URL or file provided")
	}
	...
	if *oepString != "" {
		...
		if err != nil {
			c.Println("Invalid OEP: " + err.Error())
			return err
		}
	}
	...
	case "x84":
		donutArch = donut.X84
	default:
		return errors.New("Unknown Architecture Provided")
	}
	...
	c.Println("[+] Wrote payload to %s", *srcFile)
	return nil
}
